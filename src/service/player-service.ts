...
export class PlayerService {
  ...
  async getBoardPlayers(players: string[]) {
    try {
      ...
      for (const playerId of players) {
        ...
        if (typeof player === 'string') throw new Error('Failet get player in get board players');
  ...
  async findPlayerForUser(user_id: string): Promise<string | null> {
    try {
  ...
  async updatePosition({
    ...
    newPosition,
    previous_position,
  }: DTO.PlayerUpdatePositionDTO): Promise<types.IPlayer | string> {
    try {
      const increment = previous_position > newPosition ? 200 : 0;

      const updatePlayer = await this.model.findByIdAndUpdate(
        player_id,
        {
